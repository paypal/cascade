 ...
 * @param buildVersion build version of current service
...
object StatusResponse extends LoggingSugar {
  ...
  def getStatusResponse(props: BuildProperties, serviceName: String): StatusResponse = {
    val buildVersion = props.get("service.buildVersion").getOrElse("VersionNotSet")
    ...
    val status = StatusResponse("ok", serviceName, buildVersion, dependencies.toList, gitInfo)
    logger.debug(s"Status Response - status: ${status.status}, buildVersion: $buildVersion, dependencies: ${dependencies.mkString(",")}, git-info: ${gitInfo.mkString(",")}")
    status
  }
}
