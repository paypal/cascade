...
object DummyRequestContext {
  ...
  /**
   * create a new [[RequestContext]] to pass into [[com.paypal.stingray.http.resource.ResourceHttpActor]]s for testing purposes.
   * the context will send a message to a [[ResponseHandlerActor]] when `ctx.complete` is called on it.
   * that functionality is useful for testing to ensure that the [[com.paypal.stingray.http.resource.ResourceHttpActor]] completed
