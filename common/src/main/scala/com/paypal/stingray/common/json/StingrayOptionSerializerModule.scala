...
private class StingrayOptionPropertyWriter(delegate: BeanPropertyWriter) extends BeanPropertyWriter(delegate) {
  override def serializeAsField(bean: AnyRef, jgen: JsonGenerator, prov: SerializerProvider) {
    (get(bean), _nullSerializer) match {
      case (None, _) => //suppress None in property serialization
  ...
  override def changeProperties(config: SerializationConfig,
    ...
    beanProperties.asScala.transform { w =>
      if (classOf[Option[_]].isAssignableFrom(w.getPropertyType))
        new StingrayOptionPropertyWriter(w)
      else
        w
    }.asJava
  }
...
private object StingrayOptionSerializerResolver extends Serializers.Base {
  ...
  override def findCollectionLikeSerializer(confg: SerializationConfig,
    ...
    if (!OPTION.isAssignableFrom(`type`.getRawClass))
      null
    else
      new OptionSerializer(Option(elementValueSerializer), `type`)
}

trait StingrayOptionSerializerModule extends OptionTypeModifierModule {
  this += { ctx =>
    ctx addSerializers StingrayOptionSerializerResolver
